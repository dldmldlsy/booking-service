...
public class AccommodationController {
     ...
     */
    @GetMapping("/popular")
    public ApiResponse<List<AccommodationResponse>> popular(@RequestParam(name = "limit", defaultValue = "10") int limit) {
        int safeLimit = Math.min(Math.max(limit, 1), 50);
        Map<Long, List<Availability>> availabilityByRoom = accommodationService.getAvailabilityByRoom();
        List<AccommodationResponse> data = accommodationService.getPopularAccommodations(safeLimit).stream()
                .map(acc -> toResponse(acc,
                        accommodationService.getRoomsByAccommodation(acc.getId()),
                        availabilityByRoom))
                .toList();
